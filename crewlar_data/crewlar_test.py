    ...
    for tag in soup_tag:
        ...
        if len(company):
            ...
            if company == "公司名":
                ...
                sql = "INSERT INTO {0} (company, job, location, wages, pu_time, local_code) VALUES (%s, %s, %s, %s, %s, %s)".format("jobs_table")
                if len(money):
...
def connect_db(db_tool, db_name):
    ...
    if has_table:
        ...
        for num in numbers:
            url_str = "https://search.51job.com/list/090200,000000,0000,00,9,99,%2B,2," \
                      "{0}.html?lang=c&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&ord_field=0&dibiaoid=0&line=&welfare=".format(str(num))
            print(url_str)
            request_url(url_str, crewlar_db_cursor, crewlar_db)
    else:
